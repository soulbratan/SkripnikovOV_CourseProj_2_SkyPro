...
def test_connect_to_api_success(mock_get, hh_api):
    """Тест успешного подключения к API"""
    mock_response = MagicMock()
    mock_response.raise_for_status.return_value = None
    mock_get.return_value = mock_response

    result = hh_api._HeadHunterAPI__connect_to_api()
...
@patch('src.external_api.requests.get')
def test_connect_to_api_failure(mock_get, hh_api):
...
@patch('src.external_api.requests.get')
def test_get_vacancies_connection_failed(mock_get, hh_api):
...
@patch('src.external_api.requests.get')
def test_get_vacancies_request_failed(mock_get, hh_api):
...
@patch('src.external_api.requests.get')
def test_get_vacancies_empty_response(mock_get, hh_api):
